:- dynamic finish/1.
:- dynamic faultynode/1.
:- discontiguous finish/1.

reverse([],L,L).
reverse([X|XS],L,R) :- reverse(XS,L,[X|R]).

change_finish(X):-retractall(finish(_)), assert(finish(X)).

add_faulty_node(X):- assert(faultynode(X)).

remove_faulty_node(Y):- retract(faultynode(Y)).

query_sp(Start, Finish, Path):- change_finish(Finish), find_path(Start,Path).

find_path(Start, Solution)  :-
  bfs( [[Start]], Solution).

bfs( [ [Node | Path] | _], Solution)  :-
  reverse([Node|Path],Solution,[]),
  finish(Node).

bfs( [Path | Paths], Solution)  :-
  extend( Path, NewPaths),
  append( Paths, NewPaths, Paths1),
  bfs( Paths1, Solution).

extend( [Node | Path], NewPaths)  :-
  bagof( [NewNode, Node | Path],
         ( mazelink( Node, NewNode), \+ member( NewNode, [Node | Path]) , \+ faultynode(NewNode) ),
         NewPaths),
  !.

extend( Path, [] ).             


faultynode(3).
faultynode(9).
faultynode(11).
faultynode(15).
faultynode(17).
faultynode(20).
faultynode(23).
faultynode(27).
faultynode(30).
faultynode(31).
faultynode(32).
faultynode(38).
faultynode(40).
faultynode(42).
faultynode(43).
faultynode(47).
faultynode(48).
faultynode(50).
faultynode(51).
faultynode(60).
faultynode(61).
faultynode(63).
faultynode(65).
faultynode(67).
faultynode(68).
faultynode(70).
faultynode(74).
faultynode(81).
faultynode(86).
faultynode(95).
faultynode(96).


mazelink(0,10).
mazelink(0,1).
mazelink(1,0).
mazelink(1,11).
mazelink(1,2).
mazelink(2,1).
mazelink(2,12).
mazelink(2,3).
mazelink(3,2).
mazelink(3,13).
mazelink(3,4).
mazelink(4,3).
mazelink(4,14).
mazelink(4,5).
mazelink(5,4).
mazelink(5,15).
mazelink(5,6).
mazelink(6,5).
mazelink(6,16).
mazelink(6,7).
mazelink(7,6).
mazelink(7,17).
mazelink(7,8).
mazelink(8,7).
mazelink(8,18).
mazelink(8,9).
mazelink(9,8).
mazelink(9,19).
mazelink(10,0).
mazelink(10,20).
mazelink(10,11).
mazelink(11,1).
mazelink(11,10).
mazelink(11,21).
mazelink(11,12).
mazelink(12,2).
mazelink(12,11).
mazelink(12,22).
mazelink(12,13).
mazelink(13,3).
mazelink(13,12).
mazelink(13,23).
mazelink(13,14).
mazelink(14,4).
mazelink(14,13).
mazelink(14,24).
mazelink(14,15).
mazelink(15,5).
mazelink(15,14).
mazelink(15,25).
mazelink(15,16).
mazelink(16,6).
mazelink(16,15).
mazelink(16,26).
mazelink(16,17).
mazelink(17,7).
mazelink(17,16).
mazelink(17,27).
mazelink(17,18).
mazelink(18,8).
mazelink(18,17).
mazelink(18,28).
mazelink(18,19).
mazelink(19,9).
mazelink(19,18).
mazelink(19,29).
mazelink(20,10).
mazelink(20,30).
mazelink(20,21).
mazelink(21,11).
mazelink(21,20).
mazelink(21,31).
mazelink(21,22).
mazelink(22,12).
mazelink(22,21).
mazelink(22,32).
mazelink(22,23).
mazelink(23,13).
mazelink(23,22).
mazelink(23,33).
mazelink(23,24).
mazelink(24,14).
mazelink(24,23).
mazelink(24,34).
mazelink(24,25).
mazelink(25,15).
mazelink(25,24).
mazelink(25,35).
mazelink(25,26).
mazelink(26,16).
mazelink(26,25).
mazelink(26,36).
mazelink(26,27).
mazelink(27,17).
mazelink(27,26).
mazelink(27,37).
mazelink(27,28).
mazelink(28,18).
mazelink(28,27).
mazelink(28,38).
mazelink(28,29).
mazelink(29,19).
mazelink(29,28).
mazelink(29,39).
mazelink(30,20).
mazelink(30,40).
mazelink(30,31).
mazelink(31,21).
mazelink(31,30).
mazelink(31,41).
mazelink(31,32).
mazelink(32,22).
mazelink(32,31).
mazelink(32,42).
mazelink(32,33).
mazelink(33,23).
mazelink(33,32).
mazelink(33,43).
mazelink(33,34).
mazelink(34,24).
mazelink(34,33).
mazelink(34,44).
mazelink(34,35).
mazelink(35,25).
mazelink(35,34).
mazelink(35,45).
mazelink(35,36).
mazelink(36,26).
mazelink(36,35).
mazelink(36,46).
mazelink(36,37).
mazelink(37,27).
mazelink(37,36).
mazelink(37,47).
mazelink(37,38).
mazelink(38,28).
mazelink(38,37).
mazelink(38,48).
mazelink(38,39).
mazelink(39,29).
mazelink(39,38).
mazelink(39,49).
mazelink(40,30).
mazelink(40,50).
mazelink(40,41).
mazelink(41,31).
mazelink(41,40).
mazelink(41,51).
mazelink(41,42).
mazelink(42,32).
mazelink(42,41).
mazelink(42,52).
mazelink(42,43).
mazelink(43,33).
mazelink(43,42).
mazelink(43,53).
mazelink(43,44).
mazelink(44,34).
mazelink(44,43).
mazelink(44,54).
mazelink(44,45).
mazelink(45,35).
mazelink(45,44).
mazelink(45,55).
mazelink(45,46).
mazelink(46,36).
mazelink(46,45).
mazelink(46,56).
mazelink(46,47).
mazelink(47,37).
mazelink(47,46).
mazelink(47,57).
mazelink(47,48).
mazelink(48,38).
mazelink(48,47).
mazelink(48,58).
mazelink(48,49).
mazelink(49,39).
mazelink(49,48).
mazelink(49,59).
mazelink(50,40).
mazelink(50,60).
mazelink(50,51).
mazelink(51,41).
mazelink(51,50).
mazelink(51,61).
mazelink(51,52).
mazelink(52,42).
mazelink(52,51).
mazelink(52,62).
mazelink(52,53).
mazelink(53,43).
mazelink(53,52).
mazelink(53,63).
mazelink(53,54).
mazelink(54,44).
mazelink(54,53).
mazelink(54,64).
mazelink(54,55).
mazelink(55,45).
mazelink(55,54).
mazelink(55,65).
mazelink(55,56).
mazelink(56,46).
mazelink(56,55).
mazelink(56,66).
mazelink(56,57).
mazelink(57,47).
mazelink(57,56).
mazelink(57,67).
mazelink(57,58).
mazelink(58,48).
mazelink(58,57).
mazelink(58,68).
mazelink(58,59).
mazelink(59,49).
mazelink(59,58).
mazelink(59,69).
mazelink(60,50).
mazelink(60,70).
mazelink(60,61).
mazelink(61,51).
mazelink(61,60).
mazelink(61,71).
mazelink(61,62).
mazelink(62,52).
mazelink(62,61).
mazelink(62,72).
mazelink(62,63).
mazelink(63,53).
mazelink(63,62).
mazelink(63,73).
mazelink(63,64).
mazelink(64,54).
mazelink(64,63).
mazelink(64,74).
mazelink(64,65).
mazelink(65,55).
mazelink(65,64).
mazelink(65,75).
mazelink(65,66).
mazelink(66,56).
mazelink(66,65).
mazelink(66,76).
mazelink(66,67).
mazelink(67,57).
mazelink(67,66).
mazelink(67,77).
mazelink(67,68).
mazelink(68,58).
mazelink(68,67).
mazelink(68,78).
mazelink(68,69).
mazelink(69,59).
mazelink(69,68).
mazelink(69,79).
mazelink(70,60).
mazelink(70,80).
mazelink(70,71).
mazelink(71,61).
mazelink(71,70).
mazelink(71,81).
mazelink(71,72).
mazelink(72,62).
mazelink(72,71).
mazelink(72,82).
mazelink(72,73).
mazelink(73,63).
mazelink(73,72).
mazelink(73,83).
mazelink(73,74).
mazelink(74,64).
mazelink(74,73).
mazelink(74,84).
mazelink(74,75).
mazelink(75,65).
mazelink(75,74).
mazelink(75,85).
mazelink(75,76).
mazelink(76,66).
mazelink(76,75).
mazelink(76,86).
mazelink(76,77).
mazelink(77,67).
mazelink(77,76).
mazelink(77,87).
mazelink(77,78).
mazelink(78,68).
mazelink(78,77).
mazelink(78,88).
mazelink(78,79).
mazelink(79,69).
mazelink(79,78).
mazelink(79,89).
mazelink(80,70).
mazelink(80,90).
mazelink(80,81).
mazelink(81,71).
mazelink(81,80).
mazelink(81,91).
mazelink(81,82).
mazelink(82,72).
mazelink(82,81).
mazelink(82,92).
mazelink(82,83).
mazelink(83,73).
mazelink(83,82).
mazelink(83,93).
mazelink(83,84).
mazelink(84,74).
mazelink(84,83).
mazelink(84,94).
mazelink(84,85).
mazelink(85,75).
mazelink(85,84).
mazelink(85,95).
mazelink(85,86).
mazelink(86,76).
mazelink(86,85).
mazelink(86,96).
mazelink(86,87).
mazelink(87,77).
mazelink(87,86).
mazelink(87,97).
mazelink(87,88).
mazelink(88,78).
mazelink(88,87).
mazelink(88,98).
mazelink(88,89).
mazelink(89,79).
mazelink(89,88).
mazelink(89,99).
mazelink(90,80).
mazelink(90,91).
mazelink(91,81).
mazelink(91,90).
mazelink(91,92).
mazelink(92,82).
mazelink(92,91).
mazelink(92,93).
mazelink(93,83).
mazelink(93,92).
mazelink(93,94).
mazelink(94,84).
mazelink(94,93).
mazelink(94,95).
mazelink(95,85).
mazelink(95,94).
mazelink(95,96).
mazelink(96,86).
mazelink(96,95).
mazelink(96,97).
mazelink(97,87).
mazelink(97,96).
mazelink(97,98).
mazelink(98,88).
mazelink(98,97).
mazelink(98,99).
mazelink(99,89).
mazelink(99,98).